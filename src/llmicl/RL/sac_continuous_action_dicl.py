...
from llmicl.main import iclearner
...
"""
    ...
    # else:
    #     batch_size = args.batch_size - args.llm_batch_size
        ...
        if global_step % args.interact_every == 0:
            for _ in range(args.interact_every):
                ...
                if global_step < args.learning_starts:
                    ...
                    actions, _, mean_actions = actor.get_action(torch.Tensor(obs).to(device))
                    if args.act_deterministically:
                        actions = mean_actions.detach().cpu().numpy()
                    else:
                        actions = actions.detach().cpu().numpy()
                    auxiliary_actions, _, _ = (
                        actor.get_action(torch.Tensor(obs).to(device))
                    )
                ...
                if (episode_step < args.burnin_llm) and args.add_init_burin_steps_to_llm:
                    rb_llm.add(obs, real_next_obs, actions, rewards, terminations, infos)
                episode_step += 1
            ...
            for _ in range(args.interact_every):
                ...
                if ((global_step + local_step) % args.llm_learning_frequency == 0) and (
                    ...
                    if not started_sampling:
                        ...
                        with open(f"{args.path}/runs/{run_name}/icl_started.txt", "w") as f:
                    ...
                    start_index = int(
                        np.random.randint(
                            start_episode,
                            possible_episodes_endings[random_idx] - args.context_length - 1,
                        )
                    )
                    ...
                    trainer = iclearner.MultiVariateICLTrainer(
                        model=model,
                        tokenizer=tokenizer,
                    ...
                    if args.llm_icl_parallel:
                        trainer.icl_parallel(
                            stochastic=args.stochastic_llm,
                            if_true_mean_else_mode=args.if_true_mean_else_mode,
                        )
                    else:
                        trainer.icl(
                            stochastic=args.stochastic_llm,
                    ...
                    all_llm_next_obs = copy.copy(
                        np.concatenate(
                            [
                                icl_object[dim].predictions[:].reshape((-1, 1))
                    ...
                    true_errors = np.linalg.norm(
                        all_groundtruth.squeeze() - all_llm_next_obs, axis=1
                    )

                    sorted_indices = true_errors.argsort()
                    n_to_keep = int(args.llm_percentage_to_keep * len(true_errors) / 100)
                    ...
                    for t in range(args.burnin_llm, args.context_length):
                        if t in sorted_indices[:n_to_keep]:
                            ...
                            llm_next_obs = copy.copy(
                                np.concatenate(
                                    [
                                        icl_object[dim].predictions[t].reshape((1, 1))
                                        for dim in range(n_observations)
                                    ],
                                    axis=1,
                                )
                            ...
                            # 2. Append transformed transition to augmented replay buffer
                            rb_llm.add(
                                llm_obs,
                                llm_next_obs,
                                # rb.next_observations[start_index + t],  # groundtruth
                ...
                if (
                    ...
                    if args.train_only_from_llm:
                        ...
                        #     torch.concat([data.observations, data_llm.observations], dim=0),
                        #     torch.concat([data.actions, data_llm.actions], dim=0),
                        #     torch.concat(
                        #         [data.next_observations, data_llm.next_observations], dim=0
                        #     ),
                        #     torch.concat([data.dones, data_llm.dones], dim=0),
                        #     torch.concat([data.rewards, data_llm.rewards], dim=0),
                        # )
