...
def calculate_multiPDF_llama3_parallel(
    batched_full_series,
    model,
    tokenizer,
    n_states=1000,
    ...
    batch = tokenizer(batched_full_series, return_tensors="pt", add_special_tokens=True)
    ...
    batched_PDF_list = []
    for b in range(len(batched_full_series)):
        # start_loop_from = 1 if use_instruct else 0
        PDF_list = []
        for i in range(1, int(probs.shape[1]), 2):
            PDF = MultiResolutionPDF()
            PDF.bin_center_arr = np.arange(0, 1000) / 100
            PDF.bin_width_arr = np.array(1000 * [0.01])
            PDF.bin_height_arr = probs[b, i, :].cpu().numpy() * 100
            PDF_list.append(PDF)
        batched_PDF_list.append(PDF_list)

    # release memory
    del logit_mat, kv_cache_prev  # , kv_cache_main
    return batched_PDF_list, probs, kv_cache_main
